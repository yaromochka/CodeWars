...
function calculate(expression: string): number {
    ...
    let stackOfNumbers: number[] = new Array();
    ...
    for (const indexOfToken in tokens) {
        ...
        if (isNumber(token)) {
            ...
            let resultOfExpression: number = switchOperation(token, stackOfNumbers)
...
function switchOperation(token: string, stackOfNumbers: number[]): number {
    const firstNumberInStack: number = stackOfNumbers.pop()
    const secondNumberInStack: number = stackOfNumbers.pop()
